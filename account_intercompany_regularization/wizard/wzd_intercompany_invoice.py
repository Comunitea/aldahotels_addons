...
class WzdIntercompanyRegularization(models.TransientModel):
    ...
    def _create_invoices(self, hotels, to_regularize,invoice_company):
        ...
        for hotel in hotels:
            ...
            invoice_vals = self._prepare_account_move_lines(product_ids, partner,pricelist_to_apply)
            invoice_data["invoice_line_ids"] = invoice_vals
            if invoice_data["invoice_line_ids"]:
                invoice = self.env['account.move'].create(invoice_data)
                invoices |= invoice
        return invoices

    def _prepare_account_move_lines(self, product_ids, partner,pricelist_to_apply):
        invoice_vals=[]
        for product_id in product_ids:
            ...
            pricelist_id = self.env['product.pricelist'].browse(pricelist_to_apply)
            price_unit = pricelist_id._get_product_price(product, product_ids[product_id][0])
            invoice_line = (0,0,{
